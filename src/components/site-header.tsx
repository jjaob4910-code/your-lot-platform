    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        <Link to="/" className="flex items-center gap-2 font-display text-lg font-semibold">
          <span className="grid size-5 grid-cols-2 gap-0.5">{[0,1,2,3].map(i => <span key={i} className="rounded-[2px] bg-primary" />)}</span>Your Lot
        </Link>
        ...
        <Sheet>
          ...
          <SheetContent className="w-[86%]"><SheetTitle className="font-display">Your Lot</SheetTitle><nav className="mt-10 flex flex-col gap-2">{links.map(link => <Button key={link.label} asChild variant="ghost" className="justify-start"><Link to={link.to}>{link.label}</Link></Button>)}<Button asChild className="mt-4"><Link to="/dashboard">Get started</Link></Button></nav></SheetContent>
        </Sheet>
      </div>
    </header>
  );
