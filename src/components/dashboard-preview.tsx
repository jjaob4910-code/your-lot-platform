      ...
      <div className="grid min-h-[470px] grid-cols-[54px_1fr] sm:grid-cols-[170px_1fr]">
        ...
        <div className="min-w-0 p-4 sm:p-7">
          ...
          <div className="relative mt-7 grid gap-3 pb-3 sm:min-h-[245px] sm:grid-cols-[1.2fr_.8fr] sm:pb-0">
            <div className="rounded-2xl border border-border/70 bg-card p-5 sm:pb-16"><div className="flex items-center justify-between"><p className="text-xs font-medium">Who has paid their levies</p><span className="text-xs text-muted-foreground">—</span></div><div className="mt-8 flex h-20 items-end gap-2">{[0,1,2,3,4,5,6].map(i=><span key={i} className="h-1 flex-1 rounded-full bg-muted"/>)}</div><p className="mt-4 text-[10px] text-muted-foreground">Add owners to begin tracking payments</p></div>
