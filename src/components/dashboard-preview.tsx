    ...
    <div className="soft-shadow overflow-hidden rounded-2xl border border-border/70 bg-card">
      ...
      <div className="grid min-h-[470px] grid-cols-[54px_1fr] sm:grid-cols-[170px_1fr]">
        ...
        <div className="min-w-0 p-4 sm:p-7">
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-[1.2fr_.8fr]">
            <div className="rounded-2xl border border-border/70 bg-card p-5"><div className="flex items-center justify-between"><p className="text-xs font-medium">Who has paid their levies</p><span className="text-xs text-muted-foreground">80%</span></div><div className="mt-8 flex h-20 items-end gap-2">{["h-10","h-14","h-12","h-16","h-12","h-20","h-16"].map((height,i)=><span key={i} className={`flex-1 rounded-full bg-primary/15 ${height}`}/>)}</div><p className="mt-4 text-[10px] text-muted-foreground">Two owners need a reminder</p></div>
            <div className="rounded-2xl bg-primary p-5 text-primary-foreground"><div className="flex justify-between"><p className="text-xs font-medium">Your next deadline</p><CalendarDays className="size-4 opacity-60"/></div><p className="mt-8 font-display text-4xl font-medium">18 Nov</p><p className="mt-2 text-[10px] opacity-70">Insurance renewal</p><div className="mt-5 h-px bg-primary-foreground/15"/></div>
